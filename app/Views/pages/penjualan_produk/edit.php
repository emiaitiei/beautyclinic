...
<div class="page-inner">
    <div class="d-flex align-items-left align-items-md-center flex-column flex-md-row pt-2 pb-4">
        <div>
            <h3 class="fw-bold mb-3">Edit Data Penjualan Produk</h3>
        </div>
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card">
                <div class="card-body">
                    <form action="<?= site_url('penjualan_produk/update/' . $penjualan['id_penjualan']) ?>" method="post">
                        <?= csrf_field() ?>
                        ...
                        <div class="mb-3">
                          ...
                          <select name="id_pasien" id="id_pasien" class="form-control" required>
                            <option value="">-- Pilih --</option>
                              ...
                              <option value="<?= $pasien['id_pasien'] ?>" <?= $penjualan['id_pasien'] == $pasien['id_pasien'] ? 'selected' : '' ?>>
                          ...
                          <select name="id_produk" id="id_produk" class="form-control" required>
                            <option value="">-- Pilih --</option>
                              ...
                              <option value="<?= $produk['id_produk'] ?>" data-harga="<?= $produk['harga'] ?>"
                                  <?= $penjualan['id_produk'] == $produk['id_produk'] ? 'selected' : '' ?>>
                        ...
                        <div class="mb-3">
                          ...
                          <input type="number" name="jumlah" id="jumlah" class="form-control" value="<?= $penjualan['jumlah'] ?>" min="1" required>
                        </div>

                        <div class="mb-3">
                          <label for="total_harga" class="form-label">Total Harga</label>
                          <input type="text" name="total_harga" id="total_harga" class="form-control" value="<?= $penjualan['total_harga'] ?>" required>
                        </div>

                        <div class="mb-3">
                          <label for="tanggal_pembayaran" class="form-label">Tanggal Pembayaran</label>
                          <input type="date" name="tanggal_pembayaran" class="form-control" value="<?= $penjualan['tanggal_pembayaran'] ?>" required>
                        </div>
